...
for flower in flowers:
    print(f"{flower.name} (Flower): {flower.get_height()}cm,"
          f"{flower.get_age()} days, {flower.color} color")
    flower.bloom()

for tree in trees:
    print(f"{tree.name} (Tree): {tree.get_height()}cm, {tree.get_age()} days,"
          f"{tree.trunk_diameter} color")
    tree.produce_shade()

for vegetable in vegetables:
    print(f"{vegetable.name} (Vegetable): {vegetable.get_height()}cm,"
          f"{vegetable.get_age()} days, {vegetable.harvest_season} harvest")
